...
int MTS_init(){
	...
	for (int i = 0; i < adc_tiles; i++) {
		...
		status = XRFdc_DynamicPLLConfig(&RFdcInst, ADC, i, XRFDC_EXTERNAL_CLK, 3520.0, 3520.0);
		if (status != XRFDC_SUCCESS) {
	...
	for (int i = 0; i < dac_tiles; i++) {
		...
		if (status != XRFDC_SUCCESS) {
			...
			return XRFDC_FAILURE;
				...
				status = XRFdc_DynamicPLLConfig(&RFdcInst, DAC, i, XRFDC_EXTERNAL_CLK, 3520.0, 3520.0);
				if (status != XRFDC_SUCCESS) {
					printf("could not set ADC tile %d to 3520.0 freq ret = %d\n", i, status);
				return status;
				}
