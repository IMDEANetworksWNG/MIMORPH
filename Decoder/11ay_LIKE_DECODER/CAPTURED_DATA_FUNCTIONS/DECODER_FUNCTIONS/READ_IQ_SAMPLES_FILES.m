...
switch SIGNAL.SOURCE
   ...
   case 'RFSOC'
      ...
      fclose(f);      
%       IQ_samples = complex(YY,XX)./2^15;
      IQ_samples = complex(-XX,YY)./2^15;
      
%       if SIGNAL.DEBUG_FILES
%          f=fopen([FILENAME2 '_debug.bin']);
%          PD_FLAG=double(fread(f,'*int16'));
%          fclose(f);
%          DEBUG_DATA.PD_FLAG = PD_FLAG;
%       else
%          DEBUG_DATA = [];
%       end
      ...
      DEBUG_DATA.PD_FLAG = mod(XX,2);
      DEBUG_DATA.BD_FLAG = mod(YY,2);
   case 'MATLAB'
   
      load([FILENAME2 '_CHANNEL.mat'],'IQ_samples','CHANNEL');
      DEBUG_DATA = CHANNEL;
...
%       IQ_samples = resample(signal, 352, SIGNAL.f_capture./1e7);
      DEBUG_DATA = [];
   otherwise
