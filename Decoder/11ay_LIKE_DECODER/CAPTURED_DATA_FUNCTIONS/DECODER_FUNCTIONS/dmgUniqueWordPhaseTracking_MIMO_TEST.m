function [trackedBlock, phi] = dmgUniqueWordPhaseTracking_MIMO_TEST(eqBlocks,GUARD)
...
phi = symbolPhaseError(gi,GUARD);            % Measure phase error   
trackedBlock = eqBlocks.*exp(-1i*phi); % Correct phase error
    
end

function phi = symbolPhaseError(gi,GUARD)
    Nblk = 512;
    Ngi = 64;
    numBlks = size(gi,2);
