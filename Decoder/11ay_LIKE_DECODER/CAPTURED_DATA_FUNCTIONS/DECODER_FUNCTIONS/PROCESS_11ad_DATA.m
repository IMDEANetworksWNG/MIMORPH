...
function FRAME = PROCESS_11ad_DATA(FRAME,SIGNAL,DEC_PARAM,STREAM)
...
% %    RX_FRAME = FRAME.RX_FRAME(1:DATA_LENGTH);
%    RX_FRAME = FRAME.ZF_RX_FRAME(1:DATA_LENGTH);
%    rxEqDataBlks = reshape(RX_FRAME,[512,SCEncodingInfo.NBLKS]);
%    %rxDataSym = rxEqDataBlks(DEC_PARAM.Ngi+1:end,:); 
%    rxDataSym = rxEqDataBlks(1:end-DEC_PARAM.Ngi,:); 
   rxDataSym = FRAME.ZF_RX_FRAME(1:SCEncodingInfo.NBLKS*(512-64));
   if mcsTable.NCBPS==2
      % pi/2-QPSK
%       softBits = wlanConstellationDemap(rxDataSym,FRAME.nVar_EST,mcsTable.NCBPS,-pi/4);
      SOFT_BITS = wlanConstellationDemap(rxDataSym,16/db2pow(FRAME.SNR_EST),mcsTable.NCBPS,-pi/4);
   else
      % pi/2-BPSK, pi/2-16QAM
%       softBits = wlanConstellationDemap(rxDataSym,FRAME.nVar_EST,mcsTable.NCBPS);
      SOFT_BITS = wlanConstellationDemap(rxDataSym,16/db2pow(FRAME.SNR_EST),mcsTable.NCBPS);
   end
...
elseif mcsTable.NCBPS == 4
   REF_CONST = qammod(0:2^mcsTable.NCBPS-1,2^mcsTable.NCBPS,'UnitAveragePower',true);
end
