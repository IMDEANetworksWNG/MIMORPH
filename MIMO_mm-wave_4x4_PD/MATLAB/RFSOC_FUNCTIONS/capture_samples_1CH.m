function capture_samples_1CH (ts,rx,nSamples,path,filename,SD)
    ...
    rx=channelMask(bin2dec(rx));

    buf = ['captData 0x' num2str(dec2hex(rx)) ' ' num2str(nSamples) newline];
    fwrite(ts, buf);
      ...
      if(SD==false)
         ...
         if(RES==10)
            [samplesI,samplesQ]=converTo10top(samplesI,samplesQ);
         elseif(RES==5)
            [samplesI,samplesQ]=converTo5top(samplesI,samplesQ);
         end
