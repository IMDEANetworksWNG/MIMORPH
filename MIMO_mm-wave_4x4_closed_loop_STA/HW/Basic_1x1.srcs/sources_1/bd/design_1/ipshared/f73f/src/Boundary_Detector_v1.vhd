...
--	type type_aSSRxIB is array (0 to SSR-1) of std_logic_vector(I_BITS-1 downto 0);
	...
	--b3:
        ...
        signal counter_delay_s : integer range 0 to 63 := 0;
        signal counter_s : integer range 0 to 1023 := 0;
...
--	end generate;
    ...
    b6_sm : process(clk)
        --variable counter_var : integer range 0 to 1023 := 0;
        ...
        if(rising_edge(clk)) then
            ...
            if RST_N = '0' then
                ...
                case (state) is
                    ...
                    when st1a_DELAY => 
                        counter_delay_s <= counter_delay_s + 1;
                        if counter_delay_s = 32 then
                            state <= st2_DETECTION;
                            counter_delay_s <= 0;
                        end if;
                    when st2_DETECTION =>
